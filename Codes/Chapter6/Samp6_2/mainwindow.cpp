...
void MainWindow::on_tableView_clicked(const QModelIndex &index)
{
    ...
    QString content = theModel->data(index).toString();
    labCellPos->setText(QString("当前单元格: %1 行, %2 列").arg(row+1).arg(col+1));
    labContent->setText(QString("单元格内容: %1").arg(content));
    if(dlgLocate != nullptr)
    {
        dlgLocate->setSpinValue(row, col);
    }
}
...
void MainWindow::on_actLocateCell_triggered()
{
    ...
    Qt::WindowFlags flags = dlgLocate->windowFlags(); // 获取已有的 falgs
    dlgLocate->setWindowFlags(flags | Qt::WindowStaysOnTopHint); // 显示在最上层
...
void MainWindow::setACellText(int row, int col, const QString &text)
{
...
void MainWindow::setActLocateEnabele(bool enable)
{
